...
- **一级标题 `# 本期导读`**：简短、亲切，概述当期主要趋势。
- **分类板块**：导读后直接进入二级标题分类，使用 `## 资讯`、`## 博客`（或按主题聚类）。
- **可选板块**：如有跨文章趋势，可在尾巴前增加 `## 今日观察`。
- **一级标题 `# 尾巴`**：总结并收束全文。
- **禁止项**：不要使用 `# 文章梗概和评论反响` 作为独立标题；如需概述，请放在导读末尾或分类前的 1-2 句中。
...
- **HN 观点标注**：必须明确区分原文观点、作者解读与 HN 社区观点；HN 观点使用“HN 讨论认为… / 评论区共识是…”等显式标识。
...
4. **格式检查（生成前）**：逐项检查以下内容后再输出。
   ...
   - 结构：导读后直接进入分类；不出现 `# 文章梗概和评论反响`；结尾使用 `# 尾巴`。
   - 链接：原文链接以内嵌锚文本 `[标题](url)` 出现，不单独列“原文链接/阅读更多”。
   - HN 评论：相关观点有明确来源标识（如“HN 讨论认为…”、“评论区共识是…”）。
